...
class Tamer2:
    def __init__(self, width=10, height=20):
        ...
        self.width = width
        self.height = height
        self.NUM_FEATS = 2 * (2 * self.width + 3 + self.height)
        self.record = []
        self.arecord = []
        self.frecord = []
        self.load_record = []
        ...
        self.prev_state1_feats = None  # the final state after taking an action
        self.state1_feats = None  # the final state after taking an action
    ...
    def forward(self, state_state_feats, state1_feats):
        ...
        if rid >= 8 and self.runRandom:
            ...
            if self.compiled == True:
                ...
                reward = heuristic_rewards + pred_rewards
                action = numpy.argmax(reward)
            else:
                action = numpy.random.randint(0, len(state_state_feats))
 ...
 # do a gradient update step
        if (
                human_reward != 0
                and
                prev_state_state_feats is not None
            ...
            y = numpy.reshape(numpy.array(human_reward, dtype="float32"), (1))  # add the heuristic reward
...
def GameStateEvaluation(game, events):
    ...
    if tamer == None:
        tamer = Tamer2(game.width, game.height)
        # if game.width == 10:
        #    tamer.load_weights(checkPointPath)
    ...
    if game.playAI:
        ...
        if game.feedback != 0:
            # learn from game regardless of the feedback
        ...
        if game.newFig == 1 or tamer.playAI == False or len(tamer.coa) == 0:
            ...
            tamer.prev_state1_feats = copy.deepcopy(tamer.state1_feats)
            ...
            action, actionSet = tamer.forward(state_state_feats, state1_feats)

            actionTree = copy.deepcopy(gameSym.actionTree)

            tamer.state_state_feats = state_state_feats
            tamer.state1_feats = state1_feats
            tamer.action = action
